...
class Admin extends MX_Controller {
	...
	function dashboard($days = '')
	{
		...
		// load model and libs
		$this->load->model('feedworld_model', 'feedworld');
		...
		if (!$this->feedworld->get_num_pages())
		{
		...
		// get stats
		$data['recentActivity'] = $this->feedworld->get_recent_activity();		
		$data['todaysActivity'] = $this->feedworld->get_activity('today');
		$data['yesterdaysActivity'] = $this->feedworld->get_activity('yesterday');		
		$output['activity'] = $this->parser->parse('activity_ajax', $data, TRUE);

		// get stats
		$output['days'] = (is_numeric($days)) ? $days : '30';
		$output['numPageViews'] = $this->feedworld->get_num_page_views();
		$output['numPages'] = $this->feedworld->get_num_pages();
		$output['quota'] = $this->site->get_quota();
		$output['numUsers'] = ($count = $this->feedworld->get_num_users()) ? $count : 0;
		$output['numUsersToday'] = ($count = $this->feedworld->get_num_users_today()) ? $count : 0;
		$output['numUsersYesterday'] = ($count = $this->feedworld->get_num_users_yesterday()) ? $count : 0;
		$output['numUsersWeek'] = ($count = $this->feedworld->get_num_users_week()) ? $count : 0;
		$output['numUsersLastWeek'] = ($count = $this->feedworld->get_num_users_last_week()) ? $count : 0;		
		$output['popularPages'] = $this->feedworld->get_popular_pages();	
		$output['popularShopProducts'] = $this->feedworld->get_popular_shop_products();
	...
	function activity_ajax()
	{
		...
		if ($this->session->userdata('session_admin'))
		{
			...
			// get stats
			$output['recentActivity'] = $this->feedworld->get_recent_activity();		
			$output['todaysActivity'] = $this->feedworld->get_activity('today');
			$output['yesterdaysActivity'] = $this->feedworld->get_activity('yesterday');
	...
	function backup()
	{
		...
		$filename = 'feedworld_backup_'.date('Y-m-d_H-i', time());
