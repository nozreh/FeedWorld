...
class Admin extends MX_Controller {
	...
	function edit_product($productID)
	{
		...
		{	
			...
			else
			{
				...
				// set stock alert
				if ($this->input->post('status') == 'O' || ($this->site->config['shopStockControl'] && !$this->input->post('stockAlert')))
				{
					$this->core->set['stockAlert'] = 0;
					$this->core->set['status'] = 'O';
				}
	...
	function reports($status = 'D')
	{
		// check permissions for this page
		if (!in_array('shop_reports', $this->permission->permissions))
		{
			redirect('/admin/dashboard/permissions');
		}
		...
		$this->load->view($this->includes_path.'/header');
		$this->load->view('admin/reports',$output);
		$this->load->view($this->includes_path.'/footer');
	}
