...
class Shop extends MX_Controller {
	...
	function featured()
	{
		...
		$output['user'] = $this->session->userdata('session_user');		
		$output['username'] = $this->session->userdata('username');	
	...
	function response()
	{
		...
		if ($this->site->config['shopGateway'] == 'rbsworldpay')
		{
			if ($this->shop->validate_rbsworldpay())
			{
				...
				{
					// FEEDWORLD ADD SITE
					if ($this->siteID == 1 && $this->shop->response_data['desc'] == 'FeedWorld Premium')
					{
						$this->_feedworld_premium();
					}
